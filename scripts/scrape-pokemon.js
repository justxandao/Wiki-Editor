...
function parseWikitextMoves(wikitext) {
  ...
  const pvpMatch = wikitext.match(/Movimentos\s*PVP[\s\S]*?(?====|==(?!=')|$)/i);
  const pveMatch = wikitext.match(/Movimentos\s*PVE[\s\S]*?(?====|==(?!=')|$)/i);
  const singleMovesMatch = wikitext.match(/==\s*'''Movimentos'''\s*==[\s\S]*?(?===|$)/i);
  
  function parseSection(sectionText, isPvE) {
    if (!sectionText) return;
    
    // Split by move rows - each move has two consecutive |-  align="center" rows
    // Pattern: slot row then level row
    const rowPattern = /\|-\s*align="center"[^|]*\n([\s\S]*?)(?=\|-\s*align="center"|$)/gi;
    let rowMatch;
    const rawRows = [];
    
    while ((rowMatch = rowPattern.exec(sectionText)) !== null) {
      rawRows.push(rowMatch[1]);
    }
    
    // Each move = 2 rows (row1 = slot+name+icons+element, row2 = level)
    for (let i = 0; i < rawRows.length; i += 2) {
      const row1 = rawRows[i] || '';
      const row2 = rawRows[i + 1] || '';
      
      // Extract slot from ! rowspan="2" | SLOT
      const slotMatch = row1.match(/!\s*rowspan="2"\s*\|\s*([A-Z]\d*|P)\s*\n/i);
      if (!slotMatch) continue;
      const slot = slotMatch[1].trim();
      
      // Extract move name and cooldown
      let moveName = '';
      let cooldown = '';
      let isDifferentPvE = false;
      
      // Pattern: | align="left" | Name (cd)  OR  | align="left" | '''Name (cd)'''
      const nameMatch = row1.match(/\|\s*align="left"\s*\|\s*(''')?(.+?)(?:''')?(?:\n|\|)/);
      if (nameMatch) {
        isDifferentPvE = !!nameMatch[1]; // bold = only wild
      ...
      // Extract icons from | rowspan="2" align="right" | [[Arquivo:Icon.png|Icon]]...
      const iconsMatch = row1.match(/\|\s*rowspan="2"\s*align="right"\s*\|([\s\S]*?)(?=\n\|\s*rowspan="2"|$)/);
      const icons = [];
      if (iconsMatch) {
      ...
      // Extract element and clan from | rowspan="2" | [[Arquivo:Element.png|Label|link=ClanName]]
      let element = 'Normal1';
      let clan = '';
      
      const elementMatch = row1.match(/\|\s*rowspan="2"\s*\|\s*\[\[(?:Arquivo|File):([A-Za-z0-9]+)\.(?:png|gif)[^|]*\|[^|]*\|link=([A-Za-z0-9]+)\]\]/i);
      if (elementMatch) {
        element = normalizeElement(elementMatch[1]);
        clan = elementMatch[2].trim();
      } else {
        // Alternative: try to find element without link
        const elementOnlyMatch = row1.match(/\[\[(?:Arquivo|File):([A-Za-z0-9]+)(?:\d?)\.(?:png|gif)/i);
        if (elementOnlyMatch) {
          element = normalizeElement(elementOnlyMatch[1]);
        }
      }
      
      // Extract level from second row: | align="left" | Level 100
...
async function main() {
  ...
  console.log(`\n✨ Done! ${allPokemon.length} Pokémon scraped`);
  console.log(`⏭  ${noMoves.length} skipped (no moves or redirects)`);
